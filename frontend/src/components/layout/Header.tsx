...
export default function Header(): JSX.Element {
  ...
  const isAdminSignedIn = Boolean(isAuthenticated && user?.is_admin);
  const hasAuthCookie = (): boolean =>
    typeof document !== "undefined" && /(?:^|;\s*)access_token=/.test(document.cookie);
  const isSignedIn = hasAuthCookie();

  const loginRedirectHref = (path: string): string =>
    `/login?redirect=${encodeURIComponent(path)}&required=1`;
    ...
    <header className="sticky top-0 z-20 mt-4 space-y-2">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white/90 px-4 py-4 shadow-soft backdrop-blur">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <Link
            href={isAdminSignedIn ? "/admin" : "/products"}
          ...
          <nav className="flex items-center gap-2 text-sm">
            ...
            {!isAdminSignedIn && (
              <Link
                href={isSignedIn ? "/wishlist" : loginRedirectHref("/wishlist")}
                className="rounded-full px-3 py-2 text-slate-700 hover:bg-slate-100 hover:text-slate-950"
              >
                찜 목록
              </Link>
            ...
            {!isAdminSignedIn && (
              <Link
                href={isSignedIn ? "/orders" : loginRedirectHref("/orders")}
                className="rounded-full px-3 py-2 text-slate-700 hover:bg-slate-100 hover:text-slate-950"
              >
                주문내역
              </Link>
            ...
            {!isAdminSignedIn && (
              <Link
                href={isSignedIn ? "/orders/checkout" : loginRedirectHref("/orders/checkout")}
                className="relative inline-flex items-center gap-1 rounded-full bg-brand-50 px-3 py-2 text-brand-700"
              >
